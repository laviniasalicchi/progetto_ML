...
def kfold_cv(input_vector, target_value, epochs, threshold, loss_func, eta, alfa, lambd):
    ...
    k = 10
    slice = int(np.rint(input_vector.shape[1] / k))
    ...
    for i in np.arange(0, input_vector.shape[1]+1, slice):
        ...
        if i != 0:
            ...
            accuracies = np.append(accuracies, acc)
            print(i, ")  eta:", eta, " - alfa:", alfa, " - lambda:", lambd, "** cv n",i ," TRerr:",trained_net[1],"TSerr",err,"TS ACCURACY:", acc)
            begin = i

    mean = accuracies.mean()
    print("tot accs",accuracies)
    print("mean", mean)
    return mean
